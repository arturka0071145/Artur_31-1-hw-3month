...
async def complaint(message: types.Message):
    ...
    if len(complaint_text) > 1:
        ...
        count = 1
        complaint_check = Database().sql_select_complaint_check(user_id=telegram_id,
                                                                bad_user_id=telegram_id_bad_user).fetchall()
    ...
    if len(message.text.split()) > 1:
        ...
        if user_id == message.from_user.id:
            ...
            await bot.send_message(user_id,
                                   f'Пользователь зашёл через вашу реферальную ссылку, к вам зачислено +100 баллов\n'
